...
**架构说明**：此 skill 直接协调所有子 agent。主会话调用专门的 agent（commit-symbol-analyzer、symbol-availability-checker、dependency-analyzer、symbol-origin-finder、backport-executor）。在可能的情况下，独立的 agent 会并行启动。子 agent 不会调用其他子 agent。
